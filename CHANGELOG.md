...
### Verification
- On 2026-08-10, the fixed candidate (`5c13b965710d7fafb3b7d27ddf7c85305e593bfd`) passed syntax checks for 79 JavaScript files, all 90/90 headless tests, and the release/system-boundary validator. Its validated package contained 67 clean entries (SHA-256 `a4dc9ab1b5ef538f816207b2664de03bc38a594bbd541ebf2027cd4f4f1330bb`) and emitted a manifest, checksum file, and complete file list. This checksum differs from the package actually exercised live below (`797531b8...`, built from commit `7d9b55f`) only because two doc-only commits (excluded from the archive) landed afterward and shifted the git-archive-embedded commit timestamp; the packaged source code is byte-identical between the two.
- Three of six required live Foundry matrix cells now have complete evidence: 14.365/D&D 5e 5.3.3 **passed** in full (including post-restart client reconnection), 14.365/Pathfinder 2e 8.4.0 **passed** in full, and 14.365/Custom System Builder 6.0.2 **passed its Item-transfer checks** with one distinct, still-open defect (see Known limitations above). The three Foundry 13.351 cells remain unprovisioned — Foundry 13.351 is not installed locally and obtaining it requires the maintainer's own Foundry account.
